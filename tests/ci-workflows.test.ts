  ...
  test("cross-platform CI keeps bounded jobs and immutable action references", async () => {
    ...
    const parsed = Bun.YAML.parse(workflow) as {
      jobs?: Record<string, {
        ...
        strategy?: { matrix?: { include?: Array<{ os?: string; timeout?: number }> } };
      }>;
    };

    expect(parsed.jobs?.test?.["timeout-minutes"]).toBe("${{ matrix.timeout }}");
    expect(parsed.jobs?.test?.strategy?.matrix?.include).toEqual([
      { os: "ubuntu-latest", timeout: 12 },
      { os: "macos-latest", timeout: 12 },
      { os: "windows-latest", timeout: 20 },
    ]);
    expect(parsed.jobs?.["npm-global-smoke"]?.["timeout-minutes"]).toBe(8);
