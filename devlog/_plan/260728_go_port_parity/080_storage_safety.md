...
work-phase: `wp8a`~`wp8d` (4개 하위 슬라이스, 내부 순서 있음) · 선행: 없음 · 순서 정본: `006`
...
| 5 | 프리뷰 후 선택 파일 수정/터치/삭제, 또는 `.zst` 추가·제거 → **다이제스트 재계산** | 재계산된 다이제스트가 프리뷰 다이제스트와 불일치 |
| 6 | `.trash/<epoch>/restore-pending.json` 배치 후 프리뷰 | 대기 목적지가 후보에서 제외·백필됨 |

**감사 B6 정정:** 처음에는 기준 5·6이 프리뷰를 **적용**하도록 요구했으나, 적용은 `080.2`에
와야 나온다. 자기 페이즈에서 도달할 수 없는 기준은 증거를 미루거나 지어내게 만든다.
따라서 이 슬라이스는 **순수 함수 수준**(다이제스트 재계산, 후보 필터링)까지만 단언하고,
`409 stale_preview` / `restore_pending_overlap`이라는 **HTTP 응답** 단언은 `080.2`로 옮긴다.
...
| 8 | 프리뷰 후 선택 파일 변경 → **적용** | 409 `stale_preview`, 잔여 파일·DB 행 불변 (080.1에서 이동) |
| 9 | 복원 대기 경로를 담은 낡은 다이제스트로 **적용** | `restore_pending_overlap` (080.1에서 이동) |
