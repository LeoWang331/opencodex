...
packages are uncalled duplicate/generated owners, all with deletion disposition,
not product behavior still missing from Go.
...
semantic-only. Messages token-count values also match while Go adds one trailing
newline. External providers and OS services remain outside the oracle.

**Product scenarios — about 93% data plane and 72% whole product.** This is the
...
- data-plane differential coverage is about 93%;
...
Four newly measured lifecycle/report surfaces are not included in the byte-lock:
...
- `/v1/messages/count_tokens` returns equal positive values for system,
  messages, tools, CJK, and `[1m]` inputs, but Go appends one trailing newline.

All four remain explicit semantic-only observations; none is normalized into
a false byte match.
...
|---|---:|---:|---|
| Differential scenario-family estimate | about 93% | about 72% | Weighted observable scenario inventory, not line coverage |
| Go statement coverage | 71.6% | 69.2% | Instrumented Go statements under the final R8 coverage commands |
...
a second response, and management state metrics through both real runtimes.
Messages token-count coverage adds system/tools, CJK, one-million-model marker,
and strict missing-model behavior. The remaining denominator is
...
The landed promotion set yields this 33-package scorecard, synchronized with
the authoritative repository dashboard:
...
are explicitly slated for deletion after their named canonical-owner proof;
none is intended to become a production architecture owner. Every other package
is S2 or S3.
