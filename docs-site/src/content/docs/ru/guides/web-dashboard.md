...
Для защиты от clickjacking ответы дашборда задают `X-Frame-Options: DENY` и CSP
`frame-ancestors 'none'`. Встраивание дашборда в `iframe` намеренно не поддерживается; открывайте
его как страницу верхнего уровня через `ocx gui`.
