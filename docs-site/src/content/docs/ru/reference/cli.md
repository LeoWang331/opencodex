...
удаляет принадлежащее opencodex локальное состояние только если все шаги восстановления завершились
успешно. Удаление следует манифесту владения установкой; не перечисленные в нём элементы сохраняются.
Старый непустой каталог конфигурации без манифеста, а также каталог с посторонними файлами после
удаления принадлежащих opencodex элементов остаётся для ручной проверки. Прежде чем вручную удалить
остаток, проверьте указанный командой путь. `remove` — алиас `uninstall`.
