...
| `desktop-profile.ts` | `desktop_profile.go`, `desktop_profile_parse.go` | Matched for parsing, reconciliation, family moves/defaults, applied-marker preservation, availability protection, rendering, and Claude-shaped alias guards. | Profile persistence and management endpoints call these APIs. |
| `gateway-cache.ts` | `gateway.go`, package-local atomic writer in `desktop_apply.go` | Matched for config-dir lookup, fresh-cache checks, bounded loopback proxy refresh, and atomic writes. | Production Claude launch and system-env lifecycle call `RefreshGatewayModelCacheFromProxy`. |
...
| `model-info.ts` | `modelinfo.go` | Matched for native/routed discovery, readable/Desktop IDs, capabilities, 1M context, and native effective ladders. | Production Anthropic-flavor `/v1/models` calls the canonical builder. |
...
| Responses parser/state modules used by the Go architecture | `parser.go`, `schema.go`, `envelope.go`; state lives in `internal/server/responses_state.go` | Go-specific decomposition with focused tests; state ownership matches TS `src/responses/state.ts`. | Production server expands state before invoking the pure Claude parser. |
...
The complete `internal/claude` capability set is ported, production-reachable, and activation-test locked: Messages ingress/outbound, cache affinity, Desktop profile/apply/health, model discovery, gateway lifecycle, context composition, agents, debug, Responses parsing, and reasoning envelopes. The duplicate state store is removed and production `internal/server.ResponseStateStore` matches TS ownership. Round 7 route tests also lock native effective effort ladders and profile-aware Desktop alias registry activation. Handler-owned native passthrough and orchestration divergences are tracked separately in `PRODUCTION_REACHABILITY.md`.
